...
try{
   $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
              ...
              $sql ="SELECT ref , date , statut , recettes.nom AS pizzaNom , recettes.prix AS pizzaPrix, supplements.nom as suppNom , supplements.prix AS suppPrix FROM extras INNER JOIN supplements on extras.sid = supplements.sid INNER JOIN commandes ON extras.cid = commandes.cid INNER JOIN recettes ON commandes.rid = recettes.rid  WHERE uid = ?";
              $res = $pdo-> prepare($sql);
...
try{
   $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
              $pdo=new PDO($dsn,$username,$password,$pdo_options);
              $sql ="SELECT * FROM commandes LEFT JOIN extras ON commandes.cid = extras.cid INNER JOIN recettes ON recettes.rid = commandes.rid WHERE extras.cid IS null AND uid = ?";
              $res = $pdo-> prepare($sql);
             $res ->execute (array($uid));
